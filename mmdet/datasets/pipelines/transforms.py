    ...
    def __call__(self, results):
        ...
        if random.uniform(0, 1) < self.p:
            ...
            for _ in range(n_holes):
                ...
                results['img'][y1:y2, x1:x2, :] = self.fill_in
            # img_cp= results['img']
            # img_cp = img_cp / np.max(img_cp) * 255
            # img_cp = Image.fromarray(np.uint8(img_cp)).convert('RGB')
            # img_cp.save('./test2.jpg')
            # assert 1==2
            return results
        else:
            return results
...
class GridMask:
    ...
    def __call__(self, results):
        ...
        while self.y<results['img'].shape[0]:
            self.x=temp
            while self.x<results['img'].shape[1]:
                if self.x+self.l>=results['img'].shape[1] or self.y+self.l>results['img'].shape[0]:break
                for j in range(self.x,self.x+self.l):
                    for i in range(self.y,self.y+self.l):
                        for k in range(3):
                            results['img'][i,j,k]=0
                self.x=self.x+self.l+self.w
            self.y=self.y+self.l+self.h
        return results
...
#             #mixup_image[:self.img2.shape[0], :self.img2.shape[1], :] += self.img2 * (1. - self.lambd) # ???
    ...
    def __call__(self, results):
        if self.OCP == True:
            if random.uniform(0, 1) < self.prob:
                ...
                # objectcp = object
                # objectcp = objectcp / np.max(objectcp) * 255
                # objectcp = Image.fromarray(np.uint8(objectcp)).convert('RGB')
                # objectcp.save('./test2.jpg')
                ...
                # copy_paste_image_cp = copy_paste_image
                # copy_paste_image_cp = copy_paste_image_cp / np.max(copy_paste_image_cp) * 255
                # copy_paste_image_cp = Image.fromarray(np.uint8(copy_paste_image_cp)).convert('RGB')
                # copy_paste_image_cp.save('./test1.jpg')
                # print(copy_paste_image[y1:y2,x1:x2,:].shape)
                # print(object.shape)
